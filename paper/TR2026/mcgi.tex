...
To address this challenge, we propose Manifold Consistent Graph Indexing (MCGI), a geometry aware and disk resident indexing method that leverages Local Intrinsic Dimensionality (LID) to dynamically adapt search strategies to the data's intrinsic geometry.
Unlike standard algorithms that treat dimensions uniformly, MCGI modulates its beam search budget based on in situ geometric analysis, eliminating the dependency on static hyperparameters.
...
We conducted extensive evaluations against three industry standard baselines across five datasets ranging from million to billion scales.
Empirically, MCGI achieves $5.8\times$ higher throughput at 95\% recall on the high dimensional GIST1M compared to the state of the art DiskANN.
On the billion scale SIFT1B and T2I 1B datasets, MCGI further validates its scalability by reducing high recall query latency by $3\times$ while maintaining performance parity on standard lower dimensional benchmarks.
\don{Probably need to add SPANN results}
\end{abstract}
...
State-of-the-art ANNS solutions have noticeably converged on graph-based indices, with DiskANN (Vamana)~\cite{subramanya2019diskann} being a representative example for large-scale, SSD-resident workloads. 
...
\begin{itemize}
    ...
    \item We conduct comprehensive empirical evaluations against three industry-standard baselines across five datasets ranging from million to billion scales. Results demonstrate that MCGI achieves up to $5.8\times$ higher throughput on high-dimensional vectors (e.g., 960-dimensional GIST1M) and reduces query latency by $3\times$ on the billion-scale SIFT1B compared to DiskANN, confirming its robustness in mitigating both I/O and computational bottlenecks across diverse production environments.
\end{itemize}

\section{Related Work}
\label{sec:related_work}

\don{TODO: Expand the references a bit}

\subsection{Vector Indexing Paradigms}
...
The above works largely rely on static routing parameters or centroid layouts that do not explicitly account for the local intrinsic dimensionality. 
...
\don{Expand the following paragraph}
...
\don{TODO: write a leading paragraphy to summarize our main theoretical results.}
...
\begin{itemize}
    \item RQ1 (High-Dimensional Effectiveness): How does \ours{} perform compared to baselines on high-dimensional data (e.g., GIST1M) where the curse of dimensionality typically degrades the efficiency of graph-based indices?
    \item RQ2 (High-Recall Efficiency): Can \ours{} maintain high query throughput (QPS) under strict recall requirements (e.g., Recall@10 $\ge$ 95\%), making it suitable for latency-critical production environments?
    \item RQ3 (Billion-Scale Scalability): Does the proposed method scale effectively to billion-scale datasets, including SIFT1B and T2I-1B, and demonstrate superiority over industry-standard baselines across both I/O-bound and compute-bound workloads?
    \item RQ4 (Resource Efficiency): Does the manifold-aware routing strategy translate to lower I/O overhead and reduced query latency on modern hardware (e.g., NVMe SSDs and AVX2 enabled CPUs)?
\end{itemize}
...
\begin{table}[!t]
\centering
\caption{Peak QPS at high recalls. 
}
\label{tab:high_recall}
\begin{tabular}{l|c|c}
\toprule
Algorithm & Data Locality & Recall@10 $\ge$ 95\% \\
\midrule
DiskANN~\cite{subramanya2019diskann} & disk-resident & 64.7 \\
MCGI (this work) & disk-resident & 375.1 \\
IVF-Flat~\cite{johnson2019billion} & in-memory & 590.5 \\
HNSW~\cite{malkov2018efficient} & in-memory & 2,165 \\
\bottomrule
\end{tabular}
\end{table}

HNSW establishes the performance ceiling for high-dimensional search, achieving a staggering 2,165~QPS by leveraging efficient graph traversal within the DRAM subsystem. While IVF-Flat provides a solid in-memory baseline (590.5~QPS) through contiguous scanning, the disparity between memory-resident and disk-resident methods remains vast. DiskANN, representing the state-of-the-art for disk-based indexing, faces a severe bottleneck. At 95\% recall, it plateaus at 64.7~QPS, as its static routing strategy incurs excessive random I/O when navigating the sparse high-dimensional space.

MCGI bridges this divide effectively. At 95\% recall, it delivers 375.1~QPS, outperforming the direct competitor DiskANN by a factor of $5.8\times$. Notably, MCGI's throughput begins to rival in-memory solutions, reaching approximately $64\%$ of the performance of the memory-resident IVF-Flat index. While a natural latency gap remains compared to the HNSW roofline, MCGI demonstrates that optimized I/O management can push disk-based indexing speeds into a range previously reserved for in-memory structures like inverted files.

\subsection{Billion-Scale Scalability (RQ3)}

To validate the scalability of MCGI to billion-scale datasets, we conducted comprehensive evaluations on two benchmarks: SIFT1B and T2I-1B.

Figure~\ref{fig:sift1b} illustrates the recall-QPS trade-off and latency comparisons on the SIFT1B dataset. While DiskANN maintains competitive performance at moderate recall levels, its throughput degrades sharply as retrieval requirements tighten. Specifically, at a strict recall target of approximately 90\%, DiskANN drops to 2,597 QPS. In contrast, MCGI maintains a robust throughput of 3,436 QPS, representing a $1.32\times$ speedup.
The advantage of MCGI is even more pronounced in terms of query latency, as shown in Figure~\ref{fig:sift1b} (b). At 90\% recall, DiskANN incurs a mean latency of 49.06~ms, largely due to excessive backtracking and redundant I/O operations. Conversely, MCGI leverages its geometry-aware routing to reduce the mean latency to just 16.20~ms, a $3\times$ reduction. This confirms that MCGI's manifold-consistent pruning strategy effectively identifies ``high-value'' neighbors, enabling the search agent to achieve high recall with significantly fewer disk accesses.
...
\don{Move T2I-1B experiment to appendix}
...
We presented Manifold Consistent Graph Indexing (MCGI), a framework that fundamentally rethinks vector search by aligning graph routing with the intrinsic geometry of high dimensional data. By resolving the discrepancy between Euclidean and Geodesic distances, MCGI provides a robust theoretical guarantee on connectivity and effectively mitigates the curse of dimensionality. Comprehensive evaluations on billion scale benchmarks, including SIFT1B and T2I 1B, confirm the versatility of MCGI across both I/O intensive and compute intensive scenarios. Our results demonstrate that MCGI reduces query latency by up to $3\times$ compared to the state of the art DiskANN and delivers throughput that approaches the performance of in memory inverted indices.
