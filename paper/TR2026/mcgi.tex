...
Table~\ref{tab:search_params} lists the statistical parameters derived from the dataset's LID distribution used in the Sigmoid mapping function $\Phi(\cdot)$, alongside the runtime search sweep parameters.
...
\midrule
DiskANN (disk-resident) & 64.7 & 53.8 \\
MCGI (disk-resident) & 375.1 & 83.8 \\
SPANN (disk-memory hybrid) & TBD & TBD \\
IVF-Flat (in-memory) & 590.5 & 575.6 \\
HNSW (in-memory) & 2,165 & 1,085 \\
\bottomrule
\end{tabular}
\end{table}
