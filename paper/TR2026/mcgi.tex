...
\begin{itemize}
    ...
    \item We conduct comprehensive empirical evaluations against three industry-standard baselines across five datasets ranging from million to billion scales. Results demonstrate that MCGI achieves $5.8\times$ higher throughput on high-dimensional vectors (e.g., 960-dimensional GIST1M) and reduces query latency by $3\times$ on the billion-scale SIFT1B compared to DiskANN, confirming its robustness in mitigating both I/O and computational bottlenecks across diverse production environments.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{RQ3 (Billion-Scale Scalability):} Does the proposed method scale effectively to billion-scale datasets, including SIFT1B and T2I-1B, and demonstrate superiority over industry-standard baselines across both I/O-bound and compute-bound workloads?
    \item \textbf{RQ4 (Resource Efficiency):} Does the manifold-aware routing strategy translate to lower I/O overhead and reduced query latency on modern hardware (e.g., NVMe SSDs and AVX2 enabled CPUs)?
\end{itemize}
