...
\paragraph{Datasets.}
We evaluate our method on three standard benchmark datasets that exhibit diverse characteristics in terms of dimensionality and intrinsic geometric properties. 
Summary statistics are provided in Table~\ref{tab:datasets}.

\begin{itemize}
    \item \textbf{SIFT1M}~\cite{jegou2011product}: A classic dataset of 128-dimensional image descriptors. It represents a relatively uniform distribution with low intrinsic dimensionality, serving as a baseline for general performance.
    
    \item \textbf{GIST1M}~\cite{jegou2011product}: A dataset containing 960-dimensional global image signatures. It is selected to evaluate the robustness of the index against the \textit{curse of dimensionality} and high computational cost in distance calculations.
    
    \item \textbf{GloVe-100}~\cite{pennington2014glove}: A word embedding dataset from NLP. Despite its low physical dimensionality ($D=100$), it exhibits a highly clustered distribution with varying local intrinsic dimensionalities (LID), presenting significant challenges for graph-based navigation.
\end{itemize}

\begin{table*}[t]
    \centering
    \caption{Summary of Datasets. ($N_{base}$: number of base vectors, $D$: physical dimensionality, LID: estimated Local Intrinsic Dimensionality range).}
    \label{tab:datasets}
    \begin{tabular}{lcccc}
    \toprule
    \textbf{Dataset} & \textbf{$N_{base}$} & \textbf{$D$} & \textbf{Data Type} & \textbf{Characteristics} \\
    \midrule
...
\paragraph{Baselines.}
Since MCGI is implemented within the DiskANN framework~\cite{jayaram2019diskann}, we primarily evaluate it against the standard \textit{Vamana} graph construction algorithm. 
To provide a comprehensive analysis of graph topology effects, we compare three pruning strategies:
\begin{itemize}
    \item \textbf{Strict RNG ($\alpha=1.0$):} This baseline enforces strict Relative Neighborhood Graph connectivity. It represents the heuristic strategy commonly used in the base layer of HNSW~\cite{malkov2018efficient}, prioritizing graph sparsity.
    
    \item \textbf{Vamana ($\alpha=1.2$):} The default strategy of DiskANN. It employs a relaxed pruning criteria ($\alpha=1.2$) to increase edge density, which is widely considered the state-of-the-art for flat graph indexes.
    
    \item \textbf{MCGI (Ours):} Our proposed method that dynamically adapts $\alpha$ based on the Local Intrinsic Dimensionality (LID) of each node.
\end{itemize}

\paragraph{Implementation and Hardware Environment.}
We implemented MCGI by extending the official C++ codebase of DiskANN.
All methods were compiled using GCC on Ubuntu 24.04 LTS (Linux kernel 6.8.0), with \texttt{-O3} optimization enabled.
To ensure a fair comparison and isolate algorithmic efficiency from multi-threading overhead, all latency and throughput measurements reported in the ablation studies were conducted on a single thread.
The experiments were performed on a high-performance computing server equipped with quad-socket Intel Xeon Platinum 8276 CPUs (Cascade Lake architecture).
The system features a total of 112 physical cores (224 logical threads) running at a base frequency of 2.20 GHz, with a robust 154 MiB L3 cache.
To eliminate memory bottlenecks during graph construction and index serving, the server is provisioned with 2.9 TB of DDR4 ECC memory.
For disk-based retrieval scenarios, the indices were stored on enterprise-grade KIOXIA PM5-V Series SAS SSDs (Model KPM5XVUG960G) to ensure stable I/O latency.

\subsection{Performance Comparison}
\label{subsec:performance}

We compare the search performance of MCGI against two baseline strategies:
(1) \textbf{RNG}: The strict Relative Neighborhood Graph ($\alpha=1.0$) strategy, which prioritizes sparsity but often lacks connectivity for high recall.
(2) \textbf{Vamana}: The industry-standard DiskANN baseline with relaxed pruning ($\alpha=1.2$).

Table~\ref{tab:performance} reports the maximum Queries Per Second (QPS) achievable at high-recall regimes.
On the \textbf{SIFT1M} dataset, MCGI demonstrates its efficiency. 
At the strict high-recall target (Recall $\approx$ 0.98), MCGI matches the performance of Vamana while maintaining a slightly smaller index size.
Crucially, in the practical trade-off region (Recall $\approx$ 0.95), MCGI effectively reduces redundant computations compared to the static baselines.

On the \textbf{GloVe-100} dataset, the advantages of MCGI are expected to be more pronounced due to the clustered nature of the data (results updating).
The static RNG ($\alpha=1.0$) typically fails to achieve high recall on such complex manifolds due to limited connectivity, while Vamana ($\alpha=1.2$) suffers from excessive distance computations. 
MCGI dynamically bridges this gap.

\begin{table}[h]
    \centering
    \caption{Search Performance (QPS) comparison. (\textbf{Bold} indicates best performance).}
    \label{tab:performance}
    \resizebox{\linewidth}{!}{
    \begin{tabular}{l c c | c c}
        \toprule
        & \multicolumn{2}{c}{\textbf{SIFT1M}} & \multicolumn{2}{c}{\textbf{GloVe-100}} \\
        \cmidrule(lr){2-3} \cmidrule(lr){4-5}
        Method & Recall$\approx$0.88 & Recall$\approx$0.98 & Recall$\approx$0.71 & Recall$\approx$0.88 \\
        \midrule
        Vamana (Baseline) & 145.7 & \textbf{61.5} & 145.9 & \textbf{33.5} \\
        \midrule
        \textbf{MCGI (Ours)}  & \textbf{181.9} & 61.3 & \textbf{167.2} & 33.2 \\
        \bottomrule
    \end{tabular}
    }
\end{table}

\subsection{Impact of Intrinsic Dimensionality}
\label{subsec:lid_analysis}

Instead of a traditional parameter sweep, we analyze the impact of data topology on MCGI's performance. 
Our method relies on the geometric calibration of Local Intrinsic Dimensionality (LID).
\textbf{GloVe-100} exhibits a significantly wider variance in LID compared to \textbf{SIFT1M}, reflecting its highly non-uniform manifold structure.
This explains the performance characteristics observed in Section~\ref{subsec:performance}:
on uniform distributions (SIFT1M), MCGI primarily reduces graph degree to boost speed; 
on clustered distributions (GloVe-100), the dynamic adaptation helps in navigating out of local optima, maintaining high recall where static pruning often fails.

\subsection{Indexing Efficiency}
\label{subsec:efficiency}

Finally, we verify the computational overhead introduced by the geometric calibration phase (LID estimation). 
Table~\ref{tab:construction_time} breaks down the indexing time on the \textbf{GloVe-100} dataset.
The LID estimation step is implemented with efficient multithreading and consumes a negligible fraction (approx. TBD\%) of the total construction time. 
This confirms that MCGI enhances search performance with virtually zero additional indexing cost compared to Vamana.

\begin{table}[h]
    \centering
    \caption{Construction Time Breakdown (seconds) on GloVe-100.}
    \label{tab:construction_time}
    \begin{tabular}{l r r r}
        \toprule
        Method & LID Calculation & Graph Build & \textbf{Total Time} \\
        \midrule
        Vamana & - & TBD s & TBD s \\
