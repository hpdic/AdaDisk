...
All methods are compiled with GCC 9.4 using \texttt{-O3} optimization and utilize the same AVX2 SIMD distance kernels to ensure fair comparison.
Experiments are conducted on a single thread to isolate algorithmic efficiency from parallelization overhead.
...
\begin{table}[h]
    \centering
    \caption{Search Performance (QPS) comparison. SIFT1M data is from actual experiments; GloVe-100 data is pending. (\textbf{Bold} indicates best performance).}
    \label{tab:performance}
    \resizebox{\linewidth}{!}{
    \begin{tabular}{l c c c | c c}
        \toprule
        & \multicolumn{3}{c}{\textbf{SIFT1M (Actual)}} & \multicolumn{2}{c}{\textbf{GloVe-100}} \\
        \cmidrule(lr){2-4} \cmidrule(lr){5-6}
        Method & Recall$\approx$0.88 & Recall$\approx$0.95 & Recall$\approx$0.98 & Recall$\approx$0.95 & Recall$\approx$0.98 \\
        \midrule
        RNG ($\alpha=1.0$)    & TBD & TBD & TBD & TBD & TBD \\
        Vamana ($\alpha=1.2$) & 145.7 & \textbf{122.3} & \textbf{61.5} & TBD & TBD \\
        \midrule
        \textbf{MCGI (Ours)}  & \textbf{181.9} & 101.9 & 61.3 & \textbf{TBD} & \textbf{TBD} \\
        \bottomrule
    \end{tabular}
    }
\end{table}
