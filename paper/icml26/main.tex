...
In this section, we provide the complete derivations and proofs for the theoretical propositions presented in the main methodology.

\subsection{Proof of Monotonicity (Proposition 3.4)}
\label{app:proof_monotonicity}

\textbf{Restatement.} \textit{The mapping function $\Phi$ is strictly decreasing with respect to the estimated local intrinsic dimensionality $\widehat{LID}(u)$.}

\begin{proof}
% 在这里填入 Proposition 3.4 的完整证明
Recall that the pruning parameter $\alpha(u)$ is defined via a logistic mapping on the Z-normalized LID score. Let $L = \widehat{LID}(u)$. We analyze the derivative of $\Phi$ with respect to $L$.
(TODO: detailed proof...)
The derivative is strictly negative, confirming that nodes with higher intrinsic dimensionality are assigned strictly tighter pruning constraints (smaller $\alpha$).
\end{proof}

\subsection{Proof of Boundedness (Proposition 3.5)}
\label{app:proof_boundedness}

\textbf{Restatement.} \textit{The pruning parameter $\alpha(u)$ is strictly bounded within the operational interval $[\alpha_{min}, \alpha_{max}]$.}

\begin{proof}
% 在这里填入 Proposition 3.5 的完整证明 
By construction, the logistic function $S(u)$ maps the real line to the open interval $(0, 1)$. Since $\Phi$ is a linear transformation of $S(u)$, its range is constrained by the linear bounds.
(TODO: detailed proof...)
This guarantees that the topology remains connected and does not degenerate into a disconnected graph or an overly dense complete graph.
\end{proof}
...
\begin{proof}
% 在这里填入 Lemma 4.1 的证明 
Consider the volume concentration of a hypersphere in $d$-dimensional space. As $d$ increases, the probability mass concentrates in a thin shell near the surface.
(TODO: detailed proof...)
This validates our design choice of adaptive beam width $L(q) \propto \exp(\widehat{LID}(q))$.
\end{proof}
...
% 展示 SIFT1B 结果
To further validate the engineering scalability and robustness of MCGI, we extended our evaluation to the billion-scale regime. We utilized the standard \textbf{BIGANN (SIFT1B)} benchmark, which consists of one billion 128-dimensional SIFT vectors. This experiment aims to assess whether the overhead of LID estimation and adaptive routing introduces any scalability bottlenecks when the index size scales from gigabytes to terabytes.

\subsection{Experimental Setup}
The experimental environment remains consistent with the setup detailed in Section 5.2. The index was constructed and stored on the same 480 GB Enterprise SSD. Due to storage constraints, we constructed a single index with a maximum degree $R=64$ and a construction beam width $L_{build}=100$. We compared MCGI directly against DiskANN (Vamana) using identical graph construction parameters. The evaluation metric focuses on the Recall-QPS trade-off, testing the system's ability to maintain low latency under massive I/O loads.
...
All experiments were conducted on the Chameleon Cloud platform. We provide the detailed hardware specifications below to facilitate reproducibility:
\begin{itemize}
    \item CPU: Dual Intel Xeon Platinum 8380 (Ice Lake), 80 cores total, AVX-512 support.
    \item Memory: 256 GiB DDR4 ECC RAM.
    \item Storage: Micron 5300 PRO SSD (480 GB), SATA interface.
    \item OS: Ubuntu 22.04 LTS, Kernel 5.15.
\end{itemize}

\subsection{Software Optimization}
Our implementation leverages several low-level optimizations:
\begin{itemize}
    \item \textbf{SIMD Distance Calculation:} We utilize AVX-512 intrinsics for distance computations ($L_2$ and Inner Product), achieving a significant speedup over auto-vectorized code.
    \item \textbf{Direct I/O:} To bypass the OS page cache during random reads in the search phase, we employ \texttt{O\_DIRECT} flags where applicable, ensuring that latency measurements reflect true SSD performance.
    \item \textbf{Prefetching:} We implement software prefetching (\texttt{\_mm\_prefetch}) for the graph adjacency lists to reduce memory access latency during the beam search traversal.
\end{itemize}

\section{Hyperparameter Configuration}
\label{app:hyperparameters}

We list the key hyperparameters used for MCGI construction and search in Table~\ref{tab:hyperparams}. The pruning parameters $\alpha_{min}$ and $\alpha_{max}$ were set based on the theoretical bounds discussed in Section 3.2, while the graph degree $R$ was chosen to match standard baselines.
...
\begin{tabular}{lcccc}
\toprule
Dataset & $R$ (Degree) & $L_{build}$ & $\alpha_{min}$ & $\alpha_{max}$ \\
\midrule
SIFT1M    & 64 & 100 & 1.0 & 1.5 \\
GloVe-100 & 64 & 100 & 1.0 & 1.5 \\
GIST1M    & 96 & 150 & 1.0 & 1.5 \\
SIFT1B    & 64 & 100 & 1.0 & 1.5 \\
\bottomrule
\end{tabular}
\end{table}
