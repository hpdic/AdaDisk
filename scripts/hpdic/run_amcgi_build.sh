...
ALID_DATA="${DATA_DIR}/ingest_lid.bin" 
...
# [MCGI 特有检查]
if [ ! -f "$ALID_DATA" ]; then
    echo "⚠️  警告: 未检测到 ALID 数据文件: $LID_DATA"
    echo "   如果没有这个文件，AMCGI 模块会报错或退化回默认模式。"
    echo "   (请确保你有一个计算好 ALID 并存为 .bin 格式的文件)"
