...
DISKANN_DLLEXPORT int build_merged_vamana_index(const std::string &base_file, diskann::Metric _compareMetric, uint32_t L,
                                                uint32_t R, double sampling_rate, double ram_budget,
                                                const std::string &mem_index_path, const std::string &medoids_file,
                                                const std::string &centroids_file, size_t build_pq_bytes, bool use_opq,
...
DISKANN_DLLEXPORT uint32_t optimize_beamwidth(diskann::PQFlashIndex<T, LabelT> *index_ptr,
                                              const T *tuning_sample, uint64_t tuning_sample_num,
...
int build_disk_index(const char *dataFilePath, const char *indexFilePathPrefix, const char *indexBuildParameters,
                     ...
                     const std::string &lid_file_path, float alpha_min, float alpha_max, bool use_linear = false);
